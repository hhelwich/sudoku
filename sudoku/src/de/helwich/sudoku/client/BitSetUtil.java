...
public class BitSetUtil {
	 ...
	 * 
     * To iterate over the <code>true</code> bits in a bitset,
     * use the following loop:
     * 
	 * <pre>for (int i = nextSetBit(bitset, 0); i >= 0; i = nextSetBit(bitset, i+1))
	 * 		; // do something</pre>
	 * 
	 * @param  bitset
     *         a bitset.
	 ...
	 * @return the index of the first bit that is set to <code>true</code>
	...
	private static boolean getPossiblePermutations(int[] bitsets, int[] newbitsets, int bitsetIndex, int usedBits) {
		//TODO optimize
		...
		else for (int i = nextSetBit(bitset, 0); i >= 0; i = nextSetBit(bitset, i+1)) {
			int used = set(usedBits, i);
			if (getPossiblePermutations(bitsets, newbitsets, bitsetIndex + 1, used))
