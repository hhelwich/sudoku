...
public class MatrixNodeTest {
	...
	public void setUp() throws Exception {
		...
		setAdjacents(plus, pl, pr, pu, pd);
		setAdjacents(pl, pr, plus, pl, pl);
		setAdjacents(pu, pu, pu, pd, plus);
	}
	
	private static MatrixNode cerateLonerMatrix(int row, int column) {
		MatrixNode node = new MatrixNode(row, column);
		setAdjacents(node, node, node, node, node);
		return node;
	}
	
	private static void setAdjacents(MatrixNode node, MatrixNode left, MatrixNode right,
